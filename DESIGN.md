...
Learning Path is one readable column of numbered, independently expandable steps, joined by a fine spine. A short title and plain-language summary are the skim layer; the first step opens initially. Expanded steps teach concrete behavior with short Markdown sections, highlighted code or schema examples, a takeaway, and file links. Keep detail inline instead of adding a second inspector. `DiffyMarkdownView` owns native Markdown typography and allowlisted links. `AILearningExampleView` reuses the editor's syntax colors, labels source excerpts versus simplified examples, and makes both code and its filename open the analyzed file. Use theme surfaces and restrained accent borders; no decorative metrics or completion claims. Disclosure motion respects Reduce Motion.
