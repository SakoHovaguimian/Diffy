...
- In-memory editable working-copy drafts for text fixtures, with lightweight live line matching and diff colors that refresh as the draft changes. Original comparison fixtures remain immutable and no repository files are written.
...
- Editable text comparisons use a deliberately small line matcher for prototype feedback. Drafts are scoped to the active file view and are not a production diff engine or file-writing workflow.
