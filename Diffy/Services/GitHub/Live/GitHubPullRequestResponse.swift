...
struct GitHubPullRequestResponse: Decodable, Sendable {
    ...
    var summary: PullRequestSummary {
        ...
        PullRequestSummary(id: self.id, number: self.number, title: self.title, author: self.user, assignees: self.assignees, requestedReviewers: self.requestedReviewers, requestedTeams: self.requestedTeams, isDraft: self.draft, baseRef: self.base.ref, baseSHA: self.base.sha, headRef: self.head.ref, headSHA: self.head.sha, headRepositoryFullName: self.head.repo?.fullName, createdAt: self.createdAt, updatedAt: self.updatedAt, webURL: self.htmlUrl)
